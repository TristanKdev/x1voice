...
Then roll to a second store that is deliberately different, ideally one with a different daypart mix or a heavier catering line. The failures that show up at store two are the ones that would have surprised you at scale. [Designing a voice AI pilot](/blog/designing-a-voice-ai-pilot) and [pilot exit criteria](/blog/pilot-exit-criteria-voice-ai) lay out what to agree on up front so the pilot ends in a decision rather than in a shrug.
...
Do not accept the agent reading the order back correctly as evidence. The repeat-back and the injected ticket are separate steps, and only the second one reaches the kitchen. [Why POS integration depth matters more than voice quality](/blog/why-pos-integration-depth-matters-more-than-voice-quality) is the longer version of that argument, and it is the argument that should drive the purchase.
